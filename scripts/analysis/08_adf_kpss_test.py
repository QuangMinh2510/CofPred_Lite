...
df = pd.read_csv('data\processed\gia_cafe_master_full.csv')
...
if level_nd_adf and level_nd_kpss and diff_d_adf and diff_d_kpss:
    conclusion = 'I(1): chuỗi gốc không dừng, sai phân bậc 1 dừng (ADF + KPSS đồng thuận)'
    print(f'\n✅ XÁC NHẬN {conclusion}')
elif level_nd_adf and diff_d_adf:
    conclusion = 'LIKELY I(1): ADF xác nhận cả hai chiều'
    print(f'\n✅ {conclusion}')
elif not level_nd_adf:
    conclusion = 'ADF bác bỏ nghiệm đơn vị ở chuỗi gốc — cần xem xét lại'
    print(f'\n⚠️  {conclusion}')
else:
    conclusion = 'Kết quả không đồng nhất — cần phân tích thêm'
    print(f'\n⚠️  {conclusion}')
...
rows = [
    # chuỗi gốc
    dict(series="Gia_target", transform="level", test="ADF_c",
         stat=round(s, 4),   p_approx=p,   lags=lag,
         cv_1pct=cv['1%'],   cv_5pct=cv['5%'],   cv_10pct=cv['10%'],
         stationary=not level_nd_adf),
    dict(series="Gia_target", transform="level", test="ADF_ct",
         stat=round(s2, 4),  p_approx=p2,  lags=lag2,
         cv_1pct=cv2['1%'],  cv_5pct=cv2['5%'],  cv_10pct=cv2['10%'],
         stationary=s2 < cv2['5%']),
    dict(series="Gia_target", transform="level", test="KPSS_c",
         stat=round(ks, 4),  p_approx="",  lags=klags,
         cv_1pct=kcv['1%'],  cv_5pct=kcv['5%'],  cv_10pct=kcv['10%'],
         stationary=not level_nd_kpss),
    dict(series="Gia_target", transform="level", test="KPSS_ct",
         stat=round(kts, 4), p_approx="",  lags=ktlags,
         cv_1pct=ktcv['1%'], cv_5pct=ktcv['5%'], cv_10pct=ktcv['10%'],
         stationary=kts < ktcv['5%']),
    # sai phân bậc 1
    dict(series="Gia_target", transform="diff1", test="ADF_c",
         stat=round(ds, 4),  p_approx=dp,  lags=dlag,
         cv_1pct=dcv['1%'],  cv_5pct=dcv['5%'],  cv_10pct=dcv['10%'],
         stationary=diff_d_adf),
    dict(series="Gia_target", transform="diff1", test="KPSS_c",
         stat=round(dks, 4), p_approx="",  lags=dklags,
         cv_1pct=dkcv['1%'], cv_5pct=dkcv['5%'], cv_10pct=dkcv['10%'],
         stationary=diff_d_kpss),
]

out_path = "results/statistical/adf_kpss_stationarity.csv"
