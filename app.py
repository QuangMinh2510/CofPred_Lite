...
def filter_milestone(
    ...
    """Lọc theo hôm nay, một ngày, một tháng, một năm hoặc toàn bộ quá khứ."""
    work = frame.dropna(subset=[date_column]).sort_values(date_column).copy()
    if work.empty:
        return work

    normalized_dates = work[date_column].dt.normalize()
    today = pd.Timestamp.now().normalize()

    if mode == "Hôm nay":
        result = work[normalized_dates == today]
        if result.empty:
            # File lịch sử có thể chưa có hôm nay; dùng mốc gần nhất trước hôm nay.
            result = work[normalized_dates <= today].tail(1)
        return result.copy()

    if mode == "Ngày cụ thể":
        target_date = pd.Timestamp(selected_date).normalize()
        result = work[normalized_dates == target_date]
        if result.empty:
            result = work[normalized_dates <= target_date].tail(1)
        return result.copy()

    if mode == "Theo tháng":
        target_month = pd.Period(selected_month, freq="M")
        month_values = work[date_column].dt.to_period("M")
        result = work[month_values == target_month]
        if result.empty:
            previous_months = month_values[month_values <= target_month]
            if not previous_months.empty:
                result = work[month_values == previous_months.max()]
        return result.copy()

    if mode == "Theo năm":
        year_values = work[date_column].dt.year
        result = work[year_values == int(selected_year)]
        if result.empty:
            previous_years = year_values[year_values <= int(selected_year)]
            if not previous_years.empty:
                result = work[year_values == previous_years.max()]
        return result.copy()

    return work[normalized_dates <= today].copy()
...
with overview_tab:
    overview_price_data = price_data.copy()
    if live_coffee.get("ok"):
        live_price_date = pd.to_datetime(live_coffee.get("observed_at"), errors="coerce")
        if pd.isna(live_price_date):
            live_price_date = pd.Timestamp.now().normalize()
        live_row = pd.DataFrame({
            date_col: [pd.Timestamp(live_price_date).normalize()],
            TARGET: [float(live_coffee["value"])],
        })
        overview_price_data = (
            pd.concat([overview_price_data, live_row], ignore_index=True)
    ...
    time_mode = st.selectbox(
        "Mốc thời gian",
        ["Hôm nay", "Ngày cụ thể", "Theo tháng", "Theo năm", "Tất cả ngày trước"],
        index=4,
    ...
    if time_mode == "Ngày cụ thể":
        selected_date = st.date_input(
            "Chọn ngày",
            value=pd.Timestamp.now().date(),
            min_value=overview_price_data[date_col].min().date(),
            max_value=pd.Timestamp.now().date(),
            key="overview_selected_date",
        )
    elif time_mode == "Theo tháng":
        available_months = sorted(
            overview_price_data[date_col].dt.to_period("M").dropna().unique().tolist(),
            reverse=True,
        )
    ...
    elif time_mode == "Theo năm":
        available_years = sorted(
            overview_price_data[date_col].dt.year.dropna().astype(int).unique().tolist(),
            reverse=True,
        )
        selected_year = st.selectbox(
            "Chọn năm",
            available_years,
            key="overview_selected_year",
        )

    history = filter_milestone(
        overview_price_data,
        date_col,
        time_mode,
    ...
    if history.empty:
        st.warning("Không có dữ liệu tại mốc thời gian đã chọn.")
        st.stop()

    requested_day = (
        pd.Timestamp.now().normalize()
        if time_mode == "Hôm nay"
        else pd.Timestamp(selected_date).normalize() if time_mode == "Ngày cụ thể" else None
    )
    if requested_day is not None and history[date_col].max().normalize() != requested_day:
        st.warning(
            f"Không có phiên đúng ngày {requested_day:%d/%m/%Y}; "
            f"đang dùng mốc gần nhất trước đó là {history[date_col].max():%d/%m/%Y}."
        )
    ...
    st.info(
        f"Các thẻ và biểu đồ đang được tính theo mốc {time_mode}: "
    ...
    if not PROVINCE_COFFEE_PATH.exists():
        ...
        if not selected_provinces:
            ...
            province_period = filter_milestone(
            ...
            if province_period.empty:
                st.warning(f"Không có dữ liệu tỉnh theo mốc {time_mode} đã chọn.")
            else:
    ...
    if not INTERNATIONAL_COFFEE_PATH.exists():
        ...
        international_period = filter_milestone(
            international_data,
            "Date",
            time_mode,
            selected_date=selected_date,
        ...
        if international_period.empty:
            st.warning(f"Không có dữ liệu Robusta London theo mốc {time_mode} đã chọn.")
            st.stop()
...
with prediction_tab:
    if not REGISTRY_PATH.exists():
        ...
        try:
            ...
            historical_anchor_price = float(forecast_anchor[TARGET])
            model_change = forecast - historical_anchor_price
            today = pd.Timestamp.now().normalize()
            reference_date = today
            reference_price = historical_anchor_price
            reference_source = f"giá cuối dữ liệu model ngày {forecast_anchor[date_col]:%d/%m/%Y}"

            if live_coffee.get("ok"):
                reference_price = float(live_coffee["value"])
                live_observed_date = pd.to_datetime(
                    live_coffee.get("observed_at"), errors="coerce"
                )
                if pd.notna(live_observed_date):
                    reference_date = max(today, pd.Timestamp(live_observed_date).normalize())
                    reference_source = f"giá cà phê live ngày {live_observed_date:%d/%m/%Y}"
                else:
                    reference_source = "giá cà phê live mới nhất"
                if bundle["target_mode"] == "delta":
                    # Model học mức tăng/giảm; áp mức thay đổi đó lên giá live hiện tại.
                    forecast = reference_price + model_change

            forecast_change = forecast - reference_price
            forecast_pct = forecast_change / reference_price * 100 if reference_price else 0.0
            forecast_date = reference_date + BDay(int(selected_horizon))

            p1, p2, p3, p4 = st.columns(4)
            p1.metric("Giá dự báo", format_vnd(forecast))
            p2.metric("Mốc dự báo", f"{forecast_date:%d/%m/%Y}")
            p3.metric("So với giá tham chiếu", format_vnd(forecast_change), f"{forecast_pct:+.2f}%")
            p4.metric("RMSE kiểm thử", format_vnd(float(selected["RMSE"])))

            st.caption(
                f"Ngày hiện tại: {today:%d/%m/%Y} · Tham chiếu: {reference_source} · "
                f"Model được train đến {bundle.get('trained_until', selected.get('trained_until', 'không rõ'))}."
            )
            if not live_coffee.get("ok"):
                st.warning(
                    "Không lấy được giá cà phê live nên giá dự báo vẫn dựa trên dòng dữ liệu model cuối cùng; "
                    "mốc ngày vẫn được đặt ở tương lai tính từ ngày hiện tại."
                )
...
with data_tab:
    ...
    table_data.insert(1, "Mốc ngày", table_data[date_col].dt.strftime("%d/%m/%Y"))
    table_data.insert(2, "Tháng", table_data[date_col].dt.month)
    table_data.insert(3, "Năm", table_data[date_col].dt.year)
    table_data.insert(
        4,
        "Loại mốc",
        np.where(
            table_data[date_col].dt.normalize() == pd.Timestamp.now().normalize(),
            "Hôm nay",
            "Ngày trước",
        ),
    )

    st.subheader(f"Dữ liệu theo mốc: {time_mode}")
    st.dataframe(
        table_data.tail(100).sort_values(date_col, ascending=False),
        use_container_width=True,
        hide_index=True,
    )
